...
const EMPTY_BOARD: &str = r#"{"type":"excalidraw","version":2,"source":"localboard","elements":[],"appState":{},"files":{}}"#;
...
const AGENT_PROTOCOL_VERSION: u32 = 5;
...
`*.excalidraw`, обычный JSON формата Excalidraw. Ты можешь читать эти доски,
рисовать на них новые схемы и **править уже нарисованные**. Ниже — весь
протокол; ничего, кроме него, знать не нужно.
...
node "{cli}" read "<имя доски>"   # пересказ: схемы, узлы, связи, вид, id
```
...
`read` печатает у каждого узла и каждой связи её **`id`** — это адрес элемента.
По нему делаются все правки. Там же — размеры, координаты, цвета, толщина и
шрифт: по ним видно, в каком стиле человек ведёт эту доску.

## Нарисовать новую схему

Схему кладёт само приложение — ты только оставляешь ему запрос:

```bash
node "{cli}" send --mermaid схема.mmd --title "Пайплайн" --board "Имя доски"
...
- `mermaid` — описание схемы. Координаты задавать не нужно: раскладку и
  маршруты связей считает движок раскладки по правилам построения блок-схем.
  **Веди схему сверху вниз (`flowchart TD`)**, если поток не является явно
  горизонтальным: вертикальная схема читается как текст. Направление берётся
  из первой строки описания — `TD`, `LR`, `RL`, `BT`.
- `board` — имя доски, для которой запрос. Пока её не откроют, запрос ждёт в
  ящике и в чужую доску не ляжет. **Указывай всегда**, если не уверен, что
  открыта именно нужная.
...
Связи получаются прямыми там, где блоки стоят друг под другом, и с прямыми
углами там, где нужно свернуть. Дугообразных и косых связей в этом продукте
нет намеренно: на схеме из десятка блоков кривая читается петлёй.

## Править уже нарисованную схему

Это отдельный путь, и он **не перерисовывает схему заново**. Схему могли
развести руками, раскрасить, растащить блоки — правка обязана вписаться в это.

```bash
node "{cli}" send --board "Архитектура" --ops '[
  {{"op":"insert","between":["<id>","<id>"],"text":"Проверка"}},
  {{"op":"label","id":"<id>","text":"Новая подпись"}},
  {{"op":"style","id":"<id>","from":"<id образца>"}},
  {{"op":"connect","from":"<id>","to":"<id>","like":"<id связи-образца>"}},
  {{"op":"move","id":"<id>","dx":0,"dy":-40}},
  {{"op":"delete","id":"<id>"}}
]'
```

- `insert` — вставить блок **в разрыв связи**. Существующая связь не удаляется,
  у неё переставляется конец; вторая делается её копией. Всё, что ниже места
  вставки, раздвигается ровно на высоту нового блока плюс тот зазор, который
  человек оставил между этими же двумя блоками — схема сохраняет свой ритм.
- `style` — снять вид с другого элемента: цвет, заливку, толщину, шрифт.
- `like` у `connect` — образец, с которого копируется вид связи.
- `delete` — удаляет элемент вместе с его связями.

**Вид нового элемента снимается с соседа, а не берётся из общего канона.** Если
не указать образец, им становится блок, рядом с которым идёт вставка. Поэтому
дорисованное не выбивается из схемы, даже если человек её перекрасил.

Вся правка применяется одним действием: либо все операции, либо ни одной, и
**Cmd+Z отменяет её целиком**. Удаление — это пометка, а не потеря данных.

## Правила

1. **Приложение должно быть запущено, и доска из ЭТОЙ папки открыта.** Если
   доска не та — запрос спокойно ждёт в ящике. Если запрос не разобрался,
   рядом появится файл `.error.txt` с причиной: прочитай его.
2. **Схемы приходят чёрно-белыми, и это намеренно.** Цвет на доске означает
   ...
   тебя об этом не попросили прямо. При правке цвет берётся у соседа — это не
   нарушение правила, а следование чужому выбору.
3. **Не редактируй `.excalidraw` руками.** Связи держатся привязками к фигурам;
   правка JSON снаружи их рвёт, и схема разваливается при первом же
   перетаскивании блока. Всё делается через `send`.
4. Файлы в `_inbox` приложение забирает само в течение полутора секунд.
   Чистить ничего не нужно.

## Что стоит знать про ограничения

- **Правка не перекладывает маршруты — это главное ограничение.** Обход блоков
  и разведение линий по коридорам считаются один раз, при сборке схемы. Правка
  двигает блоки, а посчитанные маршруты остаются на месте. Отсюда два
  последствия, которые надо знать заранее:
  - вставка в ветку, которая ниже сливается с соседней, может оставить связь
    соседней ветки идущей сквозь новый блок;
  - `connect` двух далёких блоков одной колонки проведёт связь напрямую, то
    есть через всё, что между ними.

  Правка годится для доработки — дописать шаг, переименовать, убрать лишнее.
  Если изменений много или схема после них поехала, честнее собрать её заново
  через `mermaid`: раскладка и маршруты посчитаются с нуля.
- **Ромб не расширяется бесконечно.** Длинное слово в подписи развилки может
  перенестись, если не влезло в полуторную ширину.
- **Вставка в обратное ребро цикла** садится на маршрут возврата и схему не
  раздвигает: там уже есть свободное место, а «ниже места вставки» для связи,
  идущей снизу вверх, ничего не значит.
- Схема, нарисованная человеком от руки (несвязанные линии, блоки, соединённые
  «на глаз»), в правки не годится: у неё нет привязок, и `read` не соберёт её в
  схему. Такую проще перерисовать, чем править.
...
- «Допиши в эту схему шаг между A и B» — `read`, затем `insert` по их `id`.
