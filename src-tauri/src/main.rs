...
const AGENT_PROTOCOL_VERSION: u32 = 4;
...
- **Правка не перекладывает маршруты.** Обход блоков и разведение линий по
  коридорам считаются при сборке схемы. Если соединить `connect`-ом два далёких
  блока одной колонки, связь пойдёт напрямую — то есть через всё, что между
  ними. Для крупной переделки честнее собрать схему заново.
