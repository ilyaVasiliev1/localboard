...
export type SchemeStyle = {
  ...
  arrow: {
    ...
    strokeWidth: number;
    /**
     * `sharp` — прямая линия от блока к блоку, `elbow` — только прямые углы с
     * автоматическим обходом, `auto` — выбрать по форме самой схемы.
     * Дугообразных вариантов нет намеренно: на схеме из десятка блоков кривая
     * читается петлёй, а не связью «отсюда сюда».
     */
    shape: "auto" | "sharp" | "elbow";
  };
...
export const DEFAULT_SCHEME_STYLE: SchemeStyle = {
  ...
  // Тип стрелки решается по схеме, а не выбирается заранее: прямая хороша в
  // линейной цепочке и начинает мешать там, где ветки расходятся и сходятся
  // обратно — тогда связи идут наискось через всю схему.
  arrow: { strokeColor: "#1e1e1e", strokeWidth: 2, shape: "auto" },
