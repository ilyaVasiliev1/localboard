...
for patch in "$PROJECT_ROOT"/patches/*.patch; do
  ...
  if ! git -C "$UPSTREAM_DIR" apply "$patch"; then
    # Молча пропущенный патч — это тихо вернувшаяся правка, которую никто не
    # заметит до следующего скриншота. Поэтому останавливаемся здесь.
    print -u2 "Патч ${patch:t} не лёг на этот коммит редактора."
    print -u2 "Почини его под новый код — или удали, если правка больше не нужна."
    exit 3
  fi
done
